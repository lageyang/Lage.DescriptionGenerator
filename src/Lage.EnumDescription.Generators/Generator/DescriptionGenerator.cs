...
public partial class DescriptionGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        IncrementalValuesProvider<TargetInfoBase?> collectorAttrs = context.SyntaxProvider.ForAttributeWithMetadataName(
            fullyQualifiedMetadataName: typeof(LageDescriptionAttribute).FullName,
            predicate: Predicate,
            transform: TransForm)
    ...
    private static TargetInfoBase? TransForm(GeneratorAttributeSyntaxContext context, CancellationToken ct)
    {
        ...
        foreach (var member in allMembers)
        {
            ...
            var descAttr = member.GetAttributes()
                .FirstOrDefault(a => a.AttributeClass?.ToDisplayString() == typeof(LageDescriptionAttribute).FullName);
    ...
    internal static void RegisterSourceOutput(SourceProductionContext spc, TargetInfoBase? item)
    {
        //生成源数据模型
        if (item is null)
            return;
